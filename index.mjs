...
const createWSolAccount = async () => {
  ...
  console.log("wsolAccount : "+wsolAddress);

  const wsolAccount = await connection.getAccountInfo(wsolAddress);

  console.log("wsol getAccountInfo : "+wsolAccount);
...
while (true) {
  ...
  if (solToUsdc.data[0].outAmount > initial+range) {
    console.log(solToUsdc.data[0].outAmount,initial+range)
    await Promise.all(
